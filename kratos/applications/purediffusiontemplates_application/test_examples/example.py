...
from KratosMultiphysics.PureDiffusionTemplatesApplication import * #and now our application. note that we can import as many as we need to solve our specific problem 


#defining a model part
model_part = ModelPart("ExampleModelPart");  #we create a model part  

import pure_diffusion_templates_solver
pure_diffusion_templates_solver.AddVariables(model_part)  #from the static_poisson_solver.py we call the function Addvariables so that the model part we have just created has the needed variables 
...
model_part.SetBufferSize(1)
 ...
 # we add the DoFs  
pure_diffusion_templates_solver.AddDofs(model_part)
   
#creating a solver object
solver = pure_diffusion_templates_solver.StaticPoissonSolver(model_part,domain_size)
solver.time_order = 1
solver.echo_level = 0
solver.Initialize()
