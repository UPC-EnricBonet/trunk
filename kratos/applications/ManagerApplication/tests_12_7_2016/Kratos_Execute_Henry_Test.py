...
import fractional_iterative_solver_henry
...
class Kratos_Execute_Test:
    ...
    def __init__(self):
        ...
        fractional_iterative_solver_henry.AddVariables(self.model_part) 

        # (note that our model part does not have nodes or elements yet) 

        # introducing input & outoput (also postProcess) file name
        output_file_name = "henry/henrySystemTest/result_henry"
        input_file_name = "henry/henrySystemTest/henry"
        ...
        # we add the DoFs  
        fractional_iterative_solver_henry.AddDofs(self.model_part)

        print ("Time steps values on each time for unkown=2 (Buffer size)") 

        #creating flow solver (custom)
        self.solver = fractional_iterative_solver_henry.FractionalIterativeSolver(self.model_part,self.domain_size)
        ...
        filePost="henry/henrySystemTest/SystemTest.txt"
