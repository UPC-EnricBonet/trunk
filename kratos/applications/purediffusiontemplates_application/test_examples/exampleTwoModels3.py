...
model_part_io = ModelPartIO("face_heat")         # we set the name of the .mdpa file  
...
#here we set the boundary conditions for the convection diffusion solver
"""for node in thermal_model_part.Nodes:
   ...
   if (node.Y<0.0):
      node.SetSolutionStepValue(POINT_HEAT_SOURCE,0,600.0)
for node in fluid_model_part.Nodes:
   node.SetSolutionStepValue(PRESSURE,0,0.0)
   if (node.Y>0.49):
      node.SetSolutionStepValue(PRESSURE,0,100.0)
   if (node.Y<0.0):
      node.SetSolutionStepValue(POINT_FLOW_SOURCE,0,600.0)"""
