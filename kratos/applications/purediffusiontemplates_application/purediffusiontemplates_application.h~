...
namespace Kratos
{
	...
	// Variables definition 
       ...
       // KRATOS_DEFINE_VARIABLE(double, CONCENTRATION_WATER)
	...
	private:
		...
		const PointSource mPointSource; //and our condition
                ...
                const KeyTemplate2D< Diffusion2D<2> > mTemplatePureDiffusion2D; // Test our template element 2D only diffusion
                const KeyTemplate2D< DiffusionConvection2D <2> > mTemplateDiffusionConvection2D; // Test our template element 2D diffusion+convection
                ...
                const DiffConv2d< Diffusion2D<2> > mTempDiffConv2d; // our template element 2D, terms convection+diffusion
                const DiffConvSt2d< DiffConv2d < Diffusion2D<2> > > mTempDiffConvSt2d; // our template element 2D, terms storage+convection+diffusion
