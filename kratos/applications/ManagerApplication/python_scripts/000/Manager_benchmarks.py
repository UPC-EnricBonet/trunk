...
import Manager_benchmarks_class as DBC   # Related to benchmarks in Chung, Ooi
...
listMPHenry = list(range(1,12))
listMPAverage  = list(range(12,13))


if benchmark_number in listMPHenry:
    import fractional_iterative_solver as Manager_parameters
elif benchmark_number in listMPAverage:
    import fractional_iterative_solver as Manager_parameters
else:
    KRATOSprint('Benchmark number does not exist')

# Strategy object
#if   (Manager_parameters.ElementType == "TriangleElement2D"     or DEM_parameters.ElementType == "TetraheElement2D"):
