...
class SolvingStrategyPython:
    ...
    def Solve(self):
        ...
        #while(it < self.max_iter and converged == False):
            ...
            #if(self.model_part.ProcessInfo.GetValue(FRACTIONAL_STEP) == 1):
                #self.StoragePressureIteration(self.model_part)
            #if(self.model_part.ProcessInfo.GetValue(FRACTIONAL_STEP) == 2):
                #self.StorageConcentrationIteration(self.model_part)

            #verify convergence
            #converged = self.convergence_criteria.PostCriteria(self.model_part,self.builder_and_solver.GetDofSet(),self.A,self.Dx,self.b)

            ####file to output
            #output = open("LastMatrixSystem.txt", "a")
            #output.write('[iteration] {0}   \n \n'.format(it))
            #output.close()
            ####
        ...
        ####file to output
        output = open("LastMatrixSystem.txt", "a")
        output.write('[A] {0}     [b] {1}      [Dx] {2} \n \n'.format(self.A,self.b,self.Dx))
        output.close()
        ####
...
##            print "RHS = ", self.b
    ...
    #######################################################################
    def StoragePressureIteration(self,model_part):
	for node in model_part.Nodes:
            node.SetSolutionStepValue(PRESSURE_OLD_IT,0,node.GetSolutionStepValue(PRESSURE,0))
 
    #######################################################################
    #######################################################################
    def StorageConcentrationIteration(self,model_part):
	for node in model_part.Nodes:
            node.SetSolutionStepValue(a,0,node.GetSolutionStepValue(CONCENTRATION,0))
 
    #######################################################################
    #######################################################################
    def CheckGlobalConverge(self,model_part):
        hasConverged = False
        maxDiffPress = 0.0
        maxDiffConc = 0.0
	for node in model_part.Nodes:
            maxDiffPress = abs ( node.GetSolutionStepValue(PRESSURE,0) - node.GetSolutionStepValue(PRESSURE_OLD_IT,0) )
            maxDiffConc = abs ( node.GetSolutionStepValue(CONCENTRATION,0) - node.GetSolutionStepValue(a,0) )
            print "maxDiffPress = ", maxDiffPress
            print "maxDiffConc = ", maxDiffConc
            if(maxDiffPress <= 0.01 and maxDiffConc <= 0.01):
                hasConverged = True
                print "CONVERGED"
            else:
                hasConverged = False
