...
import fractional_iterative_solver_henry_unitary
...
class Kratos_Execute_Test:
    ...
    def __init__(self):
        ...
        fractional_iterative_solver_henry_unitary.AddVariables(self.model_part) 
        ...
        # we add the DoFs  
        fractional_iterative_solver_henry_unitary.AddDofs(self.model_part)

        print ("Time steps values on each time for unkown=2 (Buffer size)") 

        #creating flow solver (custom)
        self.solver = fractional_iterative_solver_henry_unitary.FractionalIterativeUnitarySolver(self.model_part,self.domain_size)
