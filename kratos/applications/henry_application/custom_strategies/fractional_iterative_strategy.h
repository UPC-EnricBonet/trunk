...
public:
    ...
    double Solve()
    {
        ...
        while (!isGlobalConverged)
        {
            ...
            if( isConverged() )
            {
                ...
                /////////////////////////////////////////////////
                myfile.open ("MatrixSystem.txt", std::ios::app);
                systemfile.open ("SystemMatrix.txt", std::ios::app);
                ...
                for(int elem=0; elem<static_cast<int>(BaseType::GetModelPart().Elements().size()); elem++)
                {
                    ...
                    systemfile<< itElem->Id() << std::endl;
                    myfile << "DENSITY_ELEM   " << densityElem << std::endl;
                    systemfile<< densityElem << std::endl;
                    ...
                    systemfile<<darcyFlowX << " " <<darcyFlowY << std::endl;
   ...
   double iterationConcentration()
    ...
    void StorageOldSPressIteration()
    {
                ...
                node != BaseType::GetModelPart().NodesEnd(); ++node)
        {
            ...
            (node)->FastGetSolutionStepValue(PRESSURE_OLD_ITT) = p1;
            
            const double p2 = (node)->FastGetSolutionStepValue(PRESSURE, 1);
            (node)->FastGetSolutionStepValue(PRESSURE_OLD_ITT,1) = p2;
            
            const double p3 = (node)->GetValue(PRESSURE);
            (node)->SetValue(PRESSURE_OLD_ITT,p3);
        }
    ...
    virtual bool isConverged()
    {
        ...
        for (ModelPart::NodeIterator node = BaseType::GetModelPart().NodesBegin();
                node != BaseType::GetModelPart().NodesEnd(); ++node)
        {
            const double oldIterPressure = (node)->FastGetSolutionStepValue(PRESSURE_OLD_ITT);
    ...
    /*void ReadFile(string file, double Nsteps)
    {
         ...
         matrix<double> darcyXmatrix (Nsteps,static_cast<int>(BaseType::GetModelPart().Elements().size()));
         
         int initialSteps = Nsteps;
         ...
         for (int step=0; step<Nsteps;step++)
         {
             ...
             {
                 ModelPart::ElementsContainerType::iterator itElem = BaseType::GetModelPart().ElementsBegin()+elem;
                 Vector &varVect = itElem->GetValue(DARCY_FLOW_X_TARGET);
                 varVect.resize(initialSteps,true);
                 fscanf(fichero,"%d",&nelement);
                 fscanf(fichero,"%lf",&density);
                 fscanf(fichero,"%lf %lf",&darcyX,&darcyY);
                 darcyXmatrix (step,elem)= darcyX;
                 varVect[step] = darcyX;
                 //KRATOS_WATCH(nelement);
                 //KRATOS_WATCH(density);
                 //KRATOS_WATCH(darcyX);
                 //Vector darcyFlowX = itElem->GetValue(DARCY_FLOW_X_TARGET);
                 //itElem->SetValue(DARCY_FLOW_X_TARGET(this->mStep),darcyX);
                 //DARCY_FLOW_X_TARGET(1,1)=darcyX;
                 //KRATOS_WATCH(nelement);
                 //KRATOS_WATCH(density);
                 //KRATOS_WATCH(darcyX);
                 //KRATOS_WATCH(darcyY);
                 //KRATOS_WATCH(darcyXmatrix (this->mStep,elem));
                 //KRATOS_WATCH(itElem->GetValue(DARCY_FLOW_X_TARGET));
             }
             //KRATOS_WATCH(darcyXmatrix(step,1));
         }
         KRATOS_WATCH(darcyXmatrix(1,200)); 
         //return darcyXmatrix;
    ...
    */
    ...
    void PredictSV(int step, int prediction_order)
    {
        ...
        node != BaseType::GetModelPart().NodesEnd(); ++node)
        {
            (node)->FastGetSolutionStepValue(PRESSURE_OLD_ITT,1) = 0.0;
            (node)->FastGetSolutionStepValue(PRESSURE_OLD_ITT) = 0.0;
            (node)->SetValue(PRESSURE_OLD_ITT, 0.0);
